...
* Artefatos de planejamento e gerenciamento (PMP, EAP).
* Diagramas arquiteturais e UML (Alta Resolução).
...
* **`ADR-001.md`**: Registro formal da decisão de adoção da Arquitetura Hexagonal (*Ports and Adapters*).
* **`Diagrama_casos_de_uso.jpeg`**: Mapeamento completo das interações dos atores (Cliente, PO, Sistema Legado) com a plataforma.
* **`Diagrama_de_Componentes_implementacao.jpeg`**: Diagrama estrutural de Componentes e Implantação (React, NestJS, Postgres, MinIO).
* **`Alinhamento e Revisão do Protótipo.pdf`**: Documento oficial de feedback registrado pelo *Product Owner* (Luiz Alberto) após a apresentação do protótipo.
* **`Relatorios/`**: Documentos completo com Termo de Abertura, Planejamento, Escopo, Consolidação da Arquitetura, Validação de Protótipo e etc.
